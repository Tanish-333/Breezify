 ...
 *
 * Every response goes through withCors() — see lib/cors.ts and the sibling
 * route's own doc comment for why: the caller is always a generated app on
 * a different origin, never this one.
 */
export async function PATCH(
  ...
  if (!ID_RE.test(appId) || !ID_RE.test(collection) || !ID_RE.test(docId)) return withCors(badPath());
  ...
  if (Object.keys(fields).length === 0) {
    return withCors(NextResponse.json({ error: "Nothing to update." }, { status: 400 }));
  }
  ...
  try {
    ...
    return withCors(NextResponse.json({ id: docId, ...fields }));
  } catch (err) {
