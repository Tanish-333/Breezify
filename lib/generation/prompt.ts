...
- A favicon, or any other asset referenced from \`index.html\` with a relative path (\`<link rel="icon" href="./favicon.svg">\`, a relative \`<img src>\`, etc.), is a real build input to Vite — if that exact file isn't in "files", the real \`vite build\` fails with an unresolved-import error even though the live preview never notices (it doesn't build index.html at all). Either include the referenced file at that exact path, or sidestep the whole class of bug by inlining the favicon as a \`data:\` URI directly in the \`<link>\` tag (e.g. a tiny inline SVG) — no separate file needed. Never reference an asset path you didn't also add to "files".
