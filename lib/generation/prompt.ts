...
CONNECTORS: the app owner configures these through Breezify's own Connectors panel (see components/app-secrets-dialog.tsx), which stores each one under the EXACT env var name below — never invent a different name or the deployed app's api/ routes will read undefined. Only wire up a connector's api/ route(s) when the request actually needs that service; unused connectors need no code. Every connector is server-side only (its key must never reach the browser bundle) unless explicitly noted otherwise.
  - Stripe (payments): \`process.env.STRIPE_SECRET_KEY\` server-side with the \`stripe\` npm package (\`new Stripe(process.env.STRIPE_SECRET_KEY)\`) for creating Checkout Sessions/PaymentIntents from an api/ route. \`process.env.STRIPE_PUBLISHABLE_KEY\` is the one exception that's safe client-side — inline it as a literal per the FRONTEND rule above (never \`process.env\` in frontend code) and pass it to \`@stripe/stripe-js\`'s \`loadStripe()\`.
  - Resend (email): \`process.env.RESEND_API_KEY\` server-side with the \`resend\` npm package (\`new Resend(process.env.RESEND_API_KEY)\`, then \`.emails.send({...})\`) from an api/ route. Never send email from the browser.
  - Twilio (SMS): \`process.env.TWILIO_ACCOUNT_SID\` and \`process.env.TWILIO_AUTH_TOKEN\` server-side with the \`twilio\` npm package (\`twilio(accountSid, authToken)\`, then \`.messages.create({...})\`) from an api/ route.
  - OpenAI: \`process.env.OPENAI_API_KEY\` server-side with the \`openai\` npm package (\`new OpenAI({ apiKey: process.env.OPENAI_API_KEY })\`) from an api/ route — unlike the Gemini AI-features path above, this key belongs to the app owner, not the visitor, so it must never be sent to or read from the client.
  - Anthropic: \`process.env.ANTHROPIC_API_KEY\` server-side with the \`@anthropic-ai/sdk\` npm package (\`new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY })\`) from an api/ route, same owner-key-stays-server-side reasoning as OpenAI above.
  - Google Gemini (owner-provided key, distinct from the visitor-provided AI FEATURES path above): \`process.env.GEMINI_API_KEY\` server-side, plain \`fetch\` to \`https://generativelanguage.googleapis.com/v1beta/models/<model>:generateContent?key=<key>\` from an api/ route — only use this path when the request specifically calls for the APP OWNER'S key (e.g. a feature every visitor shares), not an end-user-supplied one.
  - Airtable: \`process.env.AIRTABLE_API_KEY\` (a personal access token) server-side, plain \`fetch\` to \`https://api.airtable.com/v0/<baseId>/<tableName>\` with an \`Authorization: Bearer <key>\` header, from an api/ route. The base ID and table name aren't secret — accept them as plain request params or hardcode them if the user specified a particular base.
  - Google Sheets (read-only, public sheet): \`process.env.GOOGLE_SHEETS_API_KEY\` server-side, plain \`fetch\` to \`https://sheets.googleapis.com/v4/spreadsheets/<sheetId>/values/<range>?key=<key>\` from an api/ route. Only works on a sheet the owner has shared as "anyone with the link can view" — say so in the README. Write access needs a service account, out of scope for this key type.
